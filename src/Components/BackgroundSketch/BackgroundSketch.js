...
import classes from './BackgroundSketch.module.css';
...
const BackgroundSketch = props => {
    ...
    const setup = (p5, canvasParentRef) => {
        ...
        // create stars
       baseLength = window.innerWidth > window.innerHeight ?
            window.innerWidth/2 :
        ...
        stellars.forEach((star) => {
            ...
            if (star.position.x <= window.innerWidth / 2) {
                star.position.x = star.position.x -
                    (window.innerWidth / 2 - star.position.x) * star.speedDegree * 0.0001;
            } else {
                star.position.x = star.position.x +
                    (star.position.x - window.innerWidth / 2) * star.speedDegree * 0.0001;
            }

            if (star.position.y <= window.innerHeight) {
                star.position.y = star.position.y -
                    (window.innerHeight / 2 - star.position.y) * star.speedDegree * 0.0001;
            } else {
                star.position.y = star.position.y +
                    (star.position.y - window.innerHeight / 2) * star.speedDegree * 0.0001;
            }
    ...
    return <Sketch className={classes.BackgroundSketch} setup={setup} draw={draw} />;
}
